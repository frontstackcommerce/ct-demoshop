...
const handleQuickViewClick = (e: Event) => {
  e.preventDefault()
  e.stopPropagation()
  emit('openQuickView')
}
...
<template>
  <div class="group/card">
    <NuxtLink :to="product.link?.path" class="block">
      <div class="flex flex-col">
        <!-- Image container -->
        <div 
          class="relative aspect-[3/4] overflow-hidden bg-shade mb-4 transition-all duration-300"
          :class="isActive ? 'ring-2 ring-foreground' : ''"
        >
          <NuxtImg 
            :src="firstImage" 
            :alt="product.name" 
            class="absolute inset-0 size-full object-cover transition-all duration-700 ease-out group-hover/card:scale-105" 
          />
          <NuxtImg
            v-if="hasSecondImage"
            :src="secondImage"
            :alt="product.name"
            class="absolute inset-0 size-full object-cover opacity-0 transition-all duration-700 ease-out group-hover/card:opacity-100 group-hover/card:scale-105"
          />
          
          <!-- Quick view overlay -->
          <div class="absolute inset-0 bg-foreground/0 group-hover/card:bg-foreground/5 transition-colors duration-500" />
          
          <!-- Active indicator -->
          <div 
            v-if="isActive"
            class="absolute top-3 right-3 size-8 bg-foreground text-background rounded-full flex items-center justify-center"
          >
            <IconCheck class="size-4" />
          </div>
          
          <!-- Quick view button -->
          <div class="absolute bottom-4 left-4 right-4 translate-y-4 opacity-0 group-hover/card:translate-y-0 group-hover/card:opacity-100 transition-all duration-500">
            <Button 
              variant="default" 
              class="w-full bg-background/95 backdrop-blur-sm text-foreground hover:bg-background text-xs tracking-wider uppercase h-10"
              :class="isActive ? 'bg-foreground text-background hover:bg-foreground/90' : ''"
        ...
        <!-- Product info -->
        <div class="space-y-1">
          <h3 class="text-sm font-medium text-foreground group-hover/card:text-primary transition-colors duration-300 line-clamp-1">
            {{ product.name }}
          </h3>
